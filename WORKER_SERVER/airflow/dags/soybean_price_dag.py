...
with DAG(
    'soybean_price_daily_to_bq',
    default_args=default_args,
    description='TradingView 대두 가격 일봉 수집 후 BigQuery 적재',
    schedule='10 16 * * *',
    catchup=True,
    max_active_runs=1,
) as dag:
    ...
    def upload_task_func(**context):
        ...
        ema_period = int(os.getenv("SOYBEAN_EMA_PERIOD", "20"))
        symbol = os.getenv("TV_SYMBOL", "ZM1!")
        exchange = os.getenv("TV_EXCHANGE", "CBOT")

        dataset_id = os.getenv("BIGQUERY_DATASET_ID", "tilda")
        table_id = os.getenv("SOYBEAN_TABLE_ID", "soybean_price")
    ...
    upload_soybean_price = PythonOperator(
        task_id='upload_soybean_price',

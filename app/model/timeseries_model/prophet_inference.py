"""
Prophet Feature 추출 (BigQuery 버전)
=====================================
BigQuery에서 데이터를 읽어서 Prophet으로 예측하고,
...
def load_data_from_bigquery(project_id, dataset_id, corn_table_id, soybean_table_id=None, end_date="2026-02-03"):
    ...
    # Soybean 데이터 로드 (Granger 검증 결과: lag8 영향)
    if soybean_table_id:
        print(f"\n📊 Soybean 데이터 로딩 중...")
        query_soybean = f"""
        SELECT 
        ...
        try:
            ...
            # 데이터 병합 (Corn 기준 left join - 결측치 그대로 유지)
            df = pd.merge(df_corn, df_soybean, on='time', how='left')
            
            missing_count = df['soybean_close'].isna().sum()
            print(f"\n✅ 데이터 병합 완료: {len(df)} 행 (Corn 기준)")
            print(f"   - Soybean 데이터 있음: {len(df) - missing_count}일")
            print(f"   - Soybean 데이터 없음: {missing_count}일")
            print(f"   → Soybean 데이터가 없는 날은 EMA, Volume만으로 예측합니다.\n")
        except Exception as e:
...
def preprocess_data(df):
    ...
    # soybean_close는 NaN이어도 괜찮음 (나중에 조건부로 사용)
    if 'soybean_close' in df.columns:
        ...
        print(f"   - Soybean 데이터 없음 (Corn만 사용): {soybean_missing}일\n")
    else:
...
def create_granger_lag_features(df):
    ...
    print("  - Soybean Close: lag 8 (데이터 있는 경우)")
    print("  - Corn EMA: lag 2 (필수)")
    print("  - Corn Volume: lag 5 (필수)")
    ...
    if 'soybean_close_lag8' in df.columns:
        ...
        print(f"   - Soybean lag8 사용 가능: {soybean_available}일")
        print(f"   - Soybean lag8 없음 (EMA, Volume만 사용): {soybean_missing}일\n")
    else:
...
def extract_prophet_features_walkforward(df, config):
    ...
    Soybean 데이터가 없는 경우 EMA, Volume만으로 예측
    """
    ...
    if has_soybean:
        print(f"   조건부 Regressor: soybean_close_lag8 (데이터 있는 경우에만 사용)")
    print()
    ...
    if has_soybean:
        ...
        print(f"   - EMA, Volume만 사용: {len(prophet_features_df) - soybean_used}일")
    else:
        print(f"   - 모든 예측에 EMA, Volume만 사용")
...
def main(project_id=None, dataset_id=None, corn_table_id=None, soybean_table_id=None, end_date="2026-02-03", output_csv=None):
    """
    메인 실행 함수
    ...
    Args:
        ...
        corn_table_id: Corn 가격 테이블 ID (None이면 기본값 'corn_price' 사용)
        soybean_table_id: Soybean 가격 테이블 ID (None이면 Soybean 없이 진행)
        end_date: 데이터 종료 날짜 (기본값: 2026-02-03)
        output_csv: 출력 CSV 파일 경로 (기본값: prophet_features_YYYYMMDD.csv)
    
    Note:
        - Granger causality 검증 결과를 반영한 Prophet feature 추출
        - Corn Close ← Soybean Close (lag 8)
        - Corn Close ← Corn EMA (lag 2)
        - Corn Close ← Corn Volume (lag 5)
    """
    print("\n" + "=" * 80)
    print("🌽 Corn Price Prediction with Granger Causality Features (BigQuery)")
    print("=" * 80)
    print("\n📋 Granger 검증 결과:")
    print("  - Corn Close ← Soybean Close (lag 8)")
    print("  - Corn Close ← Corn EMA (lag 2)")
    print("  - Corn Close ← Corn Volume (lag 5)")
    print("=" * 80 + "\n")
    ...
    if corn_table_id is None:
        # 환경변수에서 읽되, 없으면 corn_price를 기본값으로 사용
    ...
    # BigQuery에서 데이터 로드 (Corn + Soybean)
    ...
    print("\n" + "=" * 80)
    print("📊 추출된 Features 요약 (Granger 검증 기반)")
    print("=" * 80)
    ...
    print(f"\n🔬 Granger Features:")
    granger_cols = [col for col in prophet_features_df.columns if 'lag' in col or 'soybean' in col]
...
if __name__ == "__main__":
    ...
    # 기본값: .env 파일의 설정 사용
    # Corn + Soybean 데이터 사용 (Granger causality 반영)
    results = main(
        corn_table_id="corn_price",
        soybean_table_id="soybean_price",  # Soybean 테이블이 있는 경우
        end_date="2026-02-03"
    )
    
    # Soybean 없이 실행 (EMA, Volume만 사용):
    # results = main(
    #     corn_table_id="corn_price",

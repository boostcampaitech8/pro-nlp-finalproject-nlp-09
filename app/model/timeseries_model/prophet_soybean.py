"""
Prophet Feature 추출 - Soybean (BigQuery 버전)
=============================================
BigQuery에서 Soybean 데이터를 읽어서 Prophet으로 예측하고,
생성된 features를 CSV로 저장합니다.

Granger Causality 검증 결과:
- Soybean Close ← Corn Close (lag 6)
- Soybean Close ← Soybean Volume (lag 1)
- Soybean Close ← Soybean EMA (lag 1)
- Soybean Close ← Wheat Close (lag 1)
"""
...
def load_data_from_bigquery(project_id, dataset_id, soybean_table_id, corn_table_id=None, wheat_table_id=None, end_date="2026-02-03"):
    ...
    Soybean 데이터 + Corn 데이터 + Wheat 데이터 (선택적)
    """
    ...
    # Corn 데이터 로드 (Granger 검증 결과: lag6 영향)
    if corn_table_id:
        print(f"\n📊 Corn 데이터 로딩 중...")
        query_corn = f"""
        SELECT 
        ...
        try:
            ...
            # 데이터 병합 (Soybean 기준 left join - 결측치 그대로 유지)
    ...
    # Wheat 데이터 로드 (Granger 검증 결과: lag1 영향)
    if wheat_table_id:
        print(f"\n📊 Wheat 데이터 로딩 중...")
        query_wheat = f"""
        SELECT 
        ...
        try:
            ...
            missing_count = df['wheat_close'].isna().sum()
            print(f"   - Wheat 데이터 병합: {len(df) - missing_count}일 사용 가능")
        except Exception as e:
            print(f"⚠️  Wheat 데이터 로드 실패: {e}")
            print(f"   Wheat 없이 진행합니다.")
    
    print(f"\n✅ 데이터 병합 완료: {len(df)} 행 (Soybean 기준)\n")
...
def preprocess_data(df):
    ...
    # corn_close, wheat_close는 NaN이어도 괜찮음 (나중에 조건부로 사용)
    ...
    if 'wheat_close' in df.columns:
        wheat_available = df['wheat_close'].notna().sum()
        print(f"   - Wheat 데이터 있음: {wheat_available}일")
    print()
...
def create_granger_lag_features(df):
    ...
    - corn_close: lag 6 (있는 경우에만)
    - wheat_close: lag 1 (있는 경우에만)
    - Volume (Soybean): lag 1 (필수)
    - EMA (Soybean): lag 1 (필수)
    """
    df = df.copy()
    
    print("🔬 Granger 검증 결과 기반 Lag Features 생성 중...")
    print("  - Corn Close: lag 6 (데이터 있는 경우)")
    print("  - Wheat Close: lag 1 (데이터 있는 경우)")
    print("  - Soybean Volume: lag 1 (필수)")
    print("  - Soybean EMA: lag 1 (필수)")
    ...
    # 필수 컬럼(Volume_lag1, EMA_lag1)만 NaN 체크
    df = df.dropna(subset=['Volume_lag1', 'EMA_lag1']).reset_index(drop=True)
    
    # Corn, Wheat lag는 NaN이어도 괜찮음
    print(f"\n✅ Lag Features 생성 완료: {len(df)} 행")
    if 'corn_close_lag6' in df.columns:
        corn_available = df['corn_close_lag6'].notna().sum()
        print(f"   - Corn lag6 사용 가능: {corn_available}일")
    if 'wheat_close_lag1' in df.columns:
        wheat_available = df['wheat_close_lag1'].notna().sum()
        print(f"   - Wheat lag1 사용 가능: {wheat_available}일")
    print()
...
def extract_prophet_features_walkforward(df, config):
    ...
    Corn, Wheat 데이터가 없는 경우 Volume, EMA만으로 예측
    """
    ...
    if has_corn:
        print(f"   조건부 Regressor: corn_close_lag6 (데이터 있는 경우에만 사용)")
    if has_wheat:
        print(f"   조건부 Regressor: wheat_close_lag1 (데이터 있는 경우에만 사용)")
    print()
    ...
    with tqdm(total=len(df) - start_idx - 1, desc="Prophet 학습 및 예측") as pbar:
        for i in range(start_idx, len(df) - 1):
            ...
            if has_wheat and pd.notna(test_subset['wheat_close_lag1'].values[0]):
                ...
                if wheat_ratio > 0.5:  # 학습 데이터의 50% 이상에 Wheat가 있어야 사용
                    use_wheat = True
    ...
    if has_wheat:
        wheat_used = prophet_features_df['used_wheat'].sum()
        print(f"   - Wheat 포함 예측: {wheat_used}일")
...
def main(project_id=None, dataset_id=None, soybean_table_id=None, corn_table_id=None, wheat_table_id=None, end_date="2026-02-03", output_csv=None):
    """
    메인 실행 함수
    ...
    Args:
        ...
        soybean_table_id: Soybean 가격 테이블 ID (None이면 기본값 'soybean_price' 사용)
        corn_table_id: Corn 가격 테이블 ID (None이면 Corn 없이 진행)
        wheat_table_id: Wheat 가격 테이블 ID (None이면 Wheat 없이 진행)
        end_date: 데이터 종료 날짜 (기본값: 2026-02-03)
        output_csv: 출력 CSV 파일 경로 (기본값: prophet_features_soybean_YYYYMMDD_granger.csv)
    ...
    """
    print("\n" + "=" * 80)
    print("🌱 Soybean Price Prediction with Granger Causality Features (BigQuery)")
    print("=" * 80)
    print("\n📋 Granger 검증 결과:")
    print("  - Soybean Close ← Corn Close (lag 6)")
    print("  - Soybean Close ← Wheat Close (lag 1)")
    print("  - Soybean Close ← Soybean Volume (lag 1)")
    print("  - Soybean Close ← Soybean EMA (lag 1)")
    print("=" * 80 + "\n")
    ...
    # BigQuery에서 데이터 로드 (Soybean + Corn + Wheat)
    ...
    print("\n" + "=" * 80)
    print("📊 추출된 Features 요약 (Granger 검증 기반 - Soybean)")
    print("=" * 80)
    ...
    print(f"\n🔬 Granger Features:")
    granger_cols = [col for col in prophet_features_df.columns if 'lag' in col or 'corn' in col or 'wheat' in col]
...
if __name__ == "__main__":
    ...
    # 기본값: .env 파일의 설정 사용
    # Soybean + Corn + Wheat 데이터 사용 (Granger causality 반영)
    results = main(
        soybean_table_id="soybean_price",
    ...
    # Corn, Wheat 없이 실행 (Volume, EMA만 사용):
    # results = main(
    #     soybean_table_id="soybean_price",
    #     corn_table_id=None,
    #     wheat_table_id=None,

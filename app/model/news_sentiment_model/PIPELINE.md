# 옥수수 선물 가격 예측 파이프라인

## 📊 전체 워크플로우

```
┌─────────────────────────────────────────────────────────────────┐
│                    Step 0: 감성 분석 (최초 1회)                    │
│                  run_sentiment_analysis.py                       │
└─────────────────────────────────────────────────────────────────┘
                              ↓
        ┌─────────────────────────────────────────────┐
        │   news_articles_resources.csv               │
        │   (원본 뉴스: title, description, text)      │
        │   + filter_status, key_word                 │
        └─────────────────────────────────────────────┘
                              ↓
                  [자동 필터링 적용]
                  - filter_status == 'T'
                  - key_word == 'corn and (price or...)'
                              ↓
                    [FinBERT 감성 분석]
                    - GPU 환경 권장
                    - transformers 모델
                              ↓
        ┌─────────────────────────────────────────────┐
        │   corn_all_news_with_sentiment.csv          │
        │   + sentiment (positive/negative/neutral)    │
        │   + sentiment_confidence                     │
        │   + positive_score, negative_score           │
        │   + price_impact_score                       │
        │   + article_embedding (512차원)              │
        └─────────────────────────────────────────────┘


┌─────────────────────────────────────────────────────────────────┐
│                    Step 1: 모델 학습                              │
│                        train.py                                  │
└─────────────────────────────────────────────────────────────────┘
                              ↓
        ┌─────────────────────────────────────────────┐
        │   corn_all_news_with_sentiment.csv          │
        │   corn_future_price.csv                     │
        └─────────────────────────────────────────────┘
                              ↓
                  [preprocessing.py 사용]
                  1. 뉴스 데이터 전처리
                  2. 일별 뉴스 집계
                  3. 가격 데이터 전처리 (ret_1d 자동 계산)
                  4. 날짜 보정 (주말/휴일)
                  5. 뉴스-가격 병합
                  6. PCA 차원 축소 (512→50)
                  7. Lag 피처 생성
                              ↓
                    [XGBoost 학습]
                    - 클래스 불균형 처리
                    - 300 estimators
                              ↓
        ┌─────────────────────────────────────────────┐
        │   models/                                    │
        │   ├── xgb_model.json                        │
        │   ├── pca_transformer.pkl                   │
        │   └── feature_columns.json                  │
        └─────────────────────────────────────────────┘


┌─────────────────────────────────────────────────────────────────┐
│                    Step 2: 실시간 추론                            │
│                        inference.py                              │
└─────────────────────────────────────────────────────────────────┘
                              ↓
        ┌─────────────────────────────────────────────┐
        │   최근 뉴스 데이터 (3일치)                    │
        │   + 최근 가격 데이터 (5일치)                  │
        └─────────────────────────────────────────────┘
                              ↓
                  [preprocessing.py 사용]
                  - 동일한 전처리 파이프라인
                  - PCA: transform만 수행
                  - Lag 피처 생성
                              ↓
                  [저장된 모델 로드 & 예측]
                              ↓
        ┌─────────────────────────────────────────────┐
        │   예측 결과 (JSON)                           │
        │   {                                          │
        │     "prediction": 0 or 1,                   │
        │     "probability": 0.85,                    │
        │     "features_summary": {...}               │
        │   }                                          │
        └─────────────────────────────────────────────┘
                              ↓
        ┌─────────────────────────────────────────────┐
        │   LangChain → LLM 보고서 생성                │
        └─────────────────────────────────────────────┘
```

## 🔑 핵심 포인트

### 0. run_sentiment_analysis.py (감성 분석)
- **실행 시점**: 새 뉴스 수집 시에만
- **소요 시간**: 1000건당 약 5-10분 (GPU 기준)
- **출력**: 감성 점수 + 임베딩이 추가된 CSV
- **의존성**: transformers, torch
- **필터링**: filter_status='T', 특정 키워드 자동 적용

### 1. train.py (모델 학습)
- **실행 시점**: 주기적 재학습 (주 1회 또는 월 1회)
- **소요 시간**: 약 1-2분
- **출력**: 모델 파일 3개 (model, pca, features)
- **의존성**: xgboost, sklearn
- **자동 계산**: ret_1d (일일 수익률) 자동 생성

### 2. inference.py (실시간 예측)
- **실행 시점**: 매일 또는 실시간
- **소요 시간**: 1초 미만
- **출력**: JSON 예측 결과
- **의존성**: xgboost, sklearn (가벼움)

## 🎯 왜 이렇게 분리했나?

### ✅ run_sentiment_analysis.py 별도 유지 이유:
1. **비용 차이**: GPU 필요 vs CPU만으로 가능
2. **실행 빈도**: 한 번 vs 자주
3. **의존성**: 무거운 transformers vs 가벼운 sklearn
4. **재사용성**: 이미 감성 분석된 CSV 있으면 스킵

### ✅ preprocessing.py 공통 모듈화:
1. **일관성**: 학습과 추론에서 동일한 전처리
2. **유지보수**: 한 곳만 수정하면 양쪽 반영
3. **테스트**: 전처리 로직 독립적 테스트 가능
4. **자동화**: ret_1d 계산 등 반복 작업 자동화

## 💡 실제 운영 시나리오

### 시나리오 1: 최초 구축
```bash
# 1. 감성 분석 (GPU 서버에서)
python run_sentiment_analysis.py

# 2. 모델 학습
python train.py

# 3. 추론 (배포 서버에서)
python inference.py
```

### 시나리오 2: 일일 예측
```bash
# finbert.py는 스킵 (이미 감성 분석된 CSV 사용)
# train.py도 스킵 (기존 모델 사용)

# 추론만 수행
python inference.py  # 또는 API로 호출
```

### 시나리오 3: 새 뉴스 추가 + 재학습
```bash
# 1. 새 뉴스 감성 분석
python run_sentiment_analysis.py

# 2. 모델 재학습
python train.py

# 3. 새 모델로 추론
python inference.py
```

## 🔄 LangChain 통합 예시

```python
from inference import predict_next_day
import pandas as pd

# 1. 최근 데이터 로드
news_data = load_recent_news()  # 최근 3일
price_data = load_recent_prices()  # 최근 5일

# 2. 예측 수행
result = predict_next_day(news_data, price_data)

# 3. LLM 보고서 생성
prompt = f"""
다음 예측 결과를 바탕으로 옥수수 가격 전망 보고서를 작성해주세요:

예측: {'상승' if result['prediction'] == 1 else '하락'}
상승 확률: {result['probability']:.2%}

분석 데이터:
- 뉴스 기사 수: {result['features_summary']['latest_news_count']}개
- 평균 감성 점수: {result['features_summary']['avg_sentiment']:.2f}
- 가격 영향 점수: {result['features_summary']['avg_price_impact']:.2f}
- 현재 가격: ${result['features_summary']['latest_price']:.2f}
"""

report = llm.generate(prompt)
print(report)
```

...
def _parse_embedding_static(emb_str):
    """문자열로 저장된 임베딩을 numpy array로 변환 (모듈 레벨)"""
    if pd.isna(emb_str):
        return None
    try:
        if isinstance(emb_str, str):
...
def prepare_single_day_data_from_dfs(news_df, price_df, target_date, lookback_days=7):
    """
    BigQuery 등에서 가져온 DataFrame으로 특정 날짜의 예측용 전처리 데이터를 만듭니다.
    뉴스는 이미 BQ WHERE로 filter_status·keyword 필터링된 상태로 가정합니다.

    Args:
        news_df: 뉴스 DataFrame (id, publish_date, title, description, article_embedding, sentiment, ...)
        price_df: 가격 DataFrame (time, close 필수; open, high, low, Volume, EMA 있으면 사용)
        target_date: 예측 대상 날짜 (str or datetime)
        lookback_days: 과거 며칠 뉴스를 볼지

    Returns:
        dict or None: 전처리된 데이터 (뉴스 없거나 임베딩 없으면 None)
    """
    if isinstance(target_date, str):
        target_date = pd.to_datetime(target_date)
    ...
    price_row = price_df[price_df['time'] == target_date]
    if len(price_row) == 0:
        return None
    price_row = price_row.iloc[0]
    ...
    relevant_news = news_df[
        (news_df['publish_date'] >= start_date) & (news_df['publish_date'] < target_date)
    ].copy()
    ...
    for _, news in relevant_news.iterrows():
        try:
            art_emb = _parse_embedding_static(news['article_embedding'])
            if art_emb is not None:
                article_embeddings.append(art_emb)
        except Exception:
            continue
    ...
    avg_article_emb = np.mean(article_embeddings, axis=0)
    avg_entity_emb = np.zeros(1024)
    avg_triple_emb = np.zeros(1024)

    sentiment_features = {
        'avg_price_impact': float(relevant_news['price_impact_score'].mean()),
        'avg_positive': float(relevant_news['positive_score'].mean()),
        'avg_negative': float(relevant_news['negative_score'].mean()),
        'avg_neutral': float(relevant_news['neutral_score'].mean()),
        'sentiment_std': float(relevant_news['price_impact_score'].std() if len(relevant_news) > 1 else 0),
        'news_count': len(relevant_news),
        'positive_count': int((relevant_news['sentiment'] == 'positive').sum()),
        'negative_count': int((relevant_news['sentiment'] == 'negative').sum()),
        'neutral_count': int((relevant_news['sentiment'] == 'neutral').sum()),
    ...
    close = float(price_row['close'])
    open_ = float(price_row.get('open', close))
    high = float(price_row.get('high', close))
    low = float(price_row.get('low', close))
    volume = int(price_row.get('Volume', 0))
    ema = float(price_row.get('EMA', close))
    volatility = float((high - low) / close) if close else 0.0
...
def run_daily_prediction(
    ...
    """
    target_date, lookback_days와 이미 로드된 news_df, price_df로 일일 예측을 수행합니다.
    (BigQuery 등에서 get_news_for_prediction, get_price_history로 가져온 데이터 사용)

    Args:
        target_date: 예측 기준 날짜 (YYYY-MM-DD)
        lookback_days: 뉴스 lookback 일수
        news_df: 뉴스 DataFrame (corn_all_news_with_sentiment 스키마)
        price_df: 가격 DataFrame (time, close 최소)
        model_dir: 앙상블 모델 디렉토리
        output_dir: 보고서 저장 디렉토리
        save_file: True면 JSON 파일 저장

    Returns:
        dict: 예측 보고서 (실패 시 None)
    """
    predictor = EnsemblePredictor(model_dir=model_dir)
    processed_data = prepare_single_day_data_from_dfs(news_df, price_df, target_date, lookback_days=lookback_days)
    if processed_data is None:
        return None

    features = np.concatenate([
        processed_data['article_embedding'],
        processed_data['entity_embedding'],
        processed_data['triple_embedding'],
        np.array([

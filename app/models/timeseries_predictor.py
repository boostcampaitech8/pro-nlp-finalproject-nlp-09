...
def predict_market_trend(target_date: str) -> str:
    ...
    try:
        ...
        return json.dumps({"error": f"잘못된 날짜 형식입니다: '{target_date}'. YYYY-MM-DD 형식을 사용해주세요."})

    # 추론 엔진 가져오기
    try:
        engine = get_inference_engine()
    except Exception as e:
        return json.dumps({"error": f"추론 엔진 초기화 실패: {str(e)}"})

    # BigQuery에서 데이터 가져오기
    if BigQueryClient is None:
        return json.dumps({"error": "BigQueryClient를 사용할 수 없어 피처 데이터를 가져올 수 없습니다."})
    ...
    try:
        ...
        if history_df.empty:
            return json.dumps({"error": f"BigQuery에서 {target_date} (및 이전)에 대한 데이터를 찾을 수 없습니다."})

    except Exception as e:
        traceback.print_exc()  # 서버 로그 확인용
        return json.dumps({"error": f"BigQuery 데이터 조회 실패: {str(e)}"})
    ...
    try:
        ...
        return json.dumps({"error": f"예측 중 예기치 않은 오류가 발생했습니다: {str(e)}"})
